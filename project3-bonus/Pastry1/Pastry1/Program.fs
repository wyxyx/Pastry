...
let mutable failset = Set.empty<string>
    ...
    let rec loop () = actor {
        ...
        | Route(key,source,hops) ->
            if key = nodeId then
                ...
                if failset.Contains(routingTable.[rtRow,rtCol]) && rtCol <> 0 then
                    rtCol <- rtCol - 1
                ...
                if nodeId = strZero && rtCol = 0 then
                    rtCol <- rtCol + 1
...
let main argv =
    ...
    if proj3 = "project3" && numNodes > 0 && numRequests > 0 then
        printfn "Please input the number of fail nodes:"
        let line1 = System.Console.ReadLine()
        let numFail = line1 |> int
        ...
        for j=0 to numDigits-1 do
            strZero <- strZero + "0"
        let mutable fail = 0
        while fail < numFail do
            while failnode = strZero || failset.Contains(failnode) do
                failnode <- arrayKeys.[System.Random().Next() % numNodes]
            Thread.Sleep(5)
            failset <- failset.Add(failnode)
            fail <- fail + 1
        ...
        while k<= numRequests do
            for id in arrayKeys do
                //if not fail
                if failset.Contains(id)=false then
                    ctr <- ctr + 1
                    desId <- id
                    //if not in failset
                    while desId = id || failset.Contains(desId) do
